...
*/
var Kollection = function(list){
	this.items = list || [];
};

Kollection.prototype = {
	/**
	...
	*/
	collapse(){
		return new Kollection(this.reduce(function(prev, cur){
	...
	*/
	filter(callback){
		return new Kollection(this.items.filter(callback));
	},
	...
	*/
	forPage(page, items){
		...
		return new Kollection(this.items.slice((page * items) - items, page * items));
	},
	...
	*/
	intersect(items){
		...
		if(items instanceof Kollection){
			items = items.all();
		}
	...
	*/
	map(callback){
		return new Kollection(this.items.map(callback));
	},
	...
	*/
	merge(){
		return new Kollection(this.items.concat.apply(this.items, arguments));
	},
	...
	random(amount){
		...
		if(amount > 1){
			...
			for(var i = 0; i < amount; i++){
				arr.push(this.items[helpers.randomInt(0, this.items.length - 1)]);
			}

			return new Kollection(arr);
		} else {
			return this.items[helpers.randomInt(0, this.items.length - 1)];
		}
	},
	...
	*/
	reverse(){
		return new Kollection(this.items.reverse());
	},
	...
	*/
	shuffle(){
		var o = helpers.copy(this.items);

		for(var j, x, i = o.length; i; j = Math.floor(Math.random() * i), x = o[--i], o[i] = o[j], o[j] = x);

		return new Kollection(o);
	},
	...
	*/
	slice(offset, length){
		return new Kollection(this.items.slice(offset, (length) ? offset + length : this.items.length));
	},
	...
	*/
	sort(callback){
		return new Kollection(helpers.copy(this.items).sort(callback));
	},
	...
	*/
	sortBy(key){
		return new Kollection(helpers.copy(this.items).sort(function (_a, _b){
	...
	*/
	splice(index, limit, replace){
		...
		return new Kollection(this.items.splice.apply(this.items, args || arguments));
	},
	...
	*/
	take(items){
		if(items < 0){
			return new Kollection(this.items.slice(this.items.length - (-items)));
		} else {
			return new Kollection(this.items.slice(0, items));
		}
	},
	...
	*/
	values(){
		var items = helpers.copy(this.items).filter(Boolean);

		return new Kollection(items);
	},
	...
	*/
	where(key, operator, value){
		...
		return this.filter(function(item, index){
			return helpers.operate(item[key], operator, value);
		});
	},
	...
	*/
	whereStrict(key, value){
		return this.filter(function(item, index){
			return helpers.operate(item[key], '===', value);
		});
	},
